...
title = tk.Label(text="Click below to choose picture for testing disease....", background = "navy", fg="white", font=("", 15))
title.grid()
def bact():
    window.destroy()
    window1 = tk.Tk()
...
def vir():
    ...
    window1.configure(background="navy")

    def exit():
        window1.destroy()
    rem = "The remedies for Yellow leaf curl virus are: "
    remedies = tk.Label(text=rem, background="navy",
                      fg="white", font=("", 15))
    remedies.grid(column=0, row=7, padx=10, pady=10)
    ...
    remedies1 = tk.Label(text=rem1, background="navy",
                         fg="white", font=("", 12))
    remedies1.grid(column=0, row=8, padx=10, pady=10)
    button = tk.Button(text="Exit", command=exit)
    button.grid(column=0, row=9, padx=20, pady=20)
...
def openphoto():
    ...
    fileName = askopenfilename(initialdir='E:/Plant/test', title='Select image for analysis ',
                           filetypes=[('image files', '.jpg')])
    dst = "E:/Plant/testpicture"
